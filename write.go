...
import (
	...
	"github.com/needinc/go-dicom/pkg/vrraw"

	"github.com/needinc/go-dicom/pkg/uid"

	"github.com/needinc/go-dicom/pkg/dicomio"
...
var (
	...
	errorDeflatedTransferSyntaxUnsupported = errors.New("deflated explicit vr little endian transfer syntax not yet support on write (https://github.com/needinc/go-dicom/issues/323)")
)
...
func writeRawItem(w *dicomio.Writer, data []byte) error {
	if uint32(len(data))/2 != 0 {
		fmt.Println(tag.DebugString(tag.Item))
